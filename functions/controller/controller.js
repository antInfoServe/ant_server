...
class Controller {
    ...
    async getData(req, res, next) {
        ...
        try {
            if (req.query.type == undefined || req.query.orgCode == undefined) {
                this.logger().resErrorLog(req.method, 400, req.url, 'controller', 'getData', req.headers, req.body, this.apiError().badRequest(400, 'query params are not defined'))
                return next(this.apiError().badRequest(400, 'query params are not defined'))
            }

            const enums = ['configuration', 'organisation', 'role']
            if (!enums.includes(req.query.type)) {
                this.logger().resErrorLog(req.method, 400, req.url, 'controller', 'getData', req.headers, req.body, this.apiError().badRequest(400, `${req.query.type} is an invalid type value, should be configuration, organisation or role`))
                return next(this.apiError().badRequest(400, `${req.query.type} is an invalid type value, should be configuration, organisation or role`))
            }

            const result = await this.orgService().getData(req.query.type, req.query.orgCode)
            if (result.code) {
                next(this.apiError().badRequest(result.code, result.message))
                return this.logger().resErrorLog(req.method, result.code, req.url, 'controller', 'getData', req.headers, req.body, this.apiError().badRequest(result.code, result.message))
            }
            ...
            return next(this.apiError().internalServerError())
        }
    }
    ...
    async addNew(req, res, next) {
        ...
        try {
            ...
            if (valid != true) {
                this.logger().resErrorLog(req.method, 400, req.url, 'controller', 'addNew', req.headers, req.body, this.apiError().badRequest(400, valid))
                return next(this.apiError().badRequest(400, valid))
            }

            const result = await this.orgService().addNew(req.body)
            if (result.code) {
                next(this.apiError().badRequest(result.code, result.message))
                return this.logger().resErrorLog(req.method, result.code, req.url, 'controller', 'addNew', req.headers, req.body, this.apiError().badRequest(result.code, result.message))
            }
